...
- **Operator account handles (live capture 2026-05-05):**
  ...
  - **`BG - Group Management Amelia`** — third operator persona, role: "Group Management" / cash-collection / accounting layer. Distinct from Stephen03 (signal-broadcast role) and Elena03 (parallel signal-broadcast role). Avatar: a young woman; the photographic identity is presumably another paid-front-man hire on the same 白工 / CGTN-archetype pattern as Stephen Beard (§3.2). Documented role observed 2026-05-05: posting victim-supplied USDT-transfer confirmation photos to the BG-015 channel as **public proof-of-payment for the 12 % "rectification" fee**, plus textual confirmations in the form `Member ID: u<NNN> / Amount of taxes paid: $<NNN>`. This serves both as accounting and as **social-proof / peer-pressure** to push other victims into compliance.
...
- **"Professor Stephen Beard"** — claimed BGW founder, gives daily "trading signals" (delivered exclusively via BonChat per §2.4). No biographical identity verifiable outside promo materials. Referenced by name in the **FCA UK warning** (May 2025). **BonChat-handle correlation (2026-05-05 capture, see §2.4):** the broadcast-operator account name in the BG-NNN BonChat channels is literally **`BG-Stephen Beard`**, with the on-camera figure as the channel avatar. The front-man identity is therefore also the operator broadcast identity on the closed signal channel — whether operated by him personally or by someone using his name and likeness is unresolved.
  ...
  - **Visual artifacts preserved in this report:**
    - `evidence/persons/stephen-beard-zoom-still-full.jpg` — full-frame Zoom-call still showing the on-camera "Stephen Beard" persona mid-speech in a curated office set (high-res face + background).
    - `evidence/persons/stephen-beard-face-for-reverse-image-search.jpg` — tight 990×990 face crop optimized for face-search engines (PimEyes, Google Images, Yandex, TinEye).
    - **Background-set details** (visible in the full-frame): books with legible spines (*Brooklyn's Finest*, *Century…*, *Marilyn Mon…*, *IDEA AL…*), curated decorative objects (man-with-binoculars statue, abstract figural sculptures, gold trophy/star figures), an eagle-seal framed certificate bottom-right (US-state-style document — could be a genuine state-issued document, a notary commission, or a stage prop), framed diploma top-center, recessed warm under-shelf LED lighting on wood-toned shelving. Either a professional video-production set or a real office styled heavily for credibility theater. Investigators with image-similarity search access can attempt to match this set against (a) commercial-video studio rental catalogues, (b) virtual-Zoom-background libraries, (c) other Chinese-promoter content using the same room.
